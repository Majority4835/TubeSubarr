...
export async function updateSubscription(id: string, input: Prisma.ChannelUpdateInput & { userIds?: string[] }) {
  ...
  const normalizedData = {
    ...channelData,
    watchedPolicy: channelData.keepAfterWatched === false ? 'delete_immediate' : channelData.keepAfterWatched === true ? 'keep' : undefined,
    unwatchedPolicy: channelData.unwatchedRetentionDays ? 'delete_after_days' : undefined,
    unwatchedDays: typeof channelData.unwatchedRetentionDays === 'number' ? channelData.unwatchedRetentionDays : undefined,
  };
